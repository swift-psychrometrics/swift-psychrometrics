...
final class HumidityRatioTests: XCTestCase {
  ...
  func test_humidityRatio_and_partialPressure() async {
    let partialPressure = await VaporPressure(dryBulb: 75, humidity: 50%)
  ...
  func test_humidityRatio_and_vapor_pressure() async {
  ...
  func test_humidity_ratio_at_different_conditions_imperial() async {
    ...
    for (temp, expected, diff) in values {
      let ratio = await HumidityRatio(dryBulb: temp, pressure: 14.696)
      XCTApproximatelyEqual(ratio.rawValue.rawValue, expected.rawValue.rawValue, tolerance: diff)
    }
  }
  
  func test_humidity_ratio_at_different_conditions_metric() async {
    ...
    for (temp, expected, diff) in values {
      let ratio = await HumidityRatio(dryBulb: .celsius(temp), pressure: .pascals(101325))
      XCTApproximatelyEqual(ratio.rawValue.rawValue, expected.rawValue.rawValue, tolerance: diff)
    }
  }

  func test_humidityRatio_and_relativeHumidity_imperial() async {
    let relativeHumidity = await RelativeHumidity.init(dryBulb: 100, ratio: 0.00523, pressure: 14.696, units: .imperial)
