...
final class WetBulbTests: PsychrometricTestCase {
  ...
  func test_humidityRatio_and_wetBulb_imperial() async throws {
    ...
    // Above freezing.
    var ratio = try await client.humidityRatio(
      .wetBulb(
        77,
        dryBulb: 86,
    ...
    var wetBulb = try await client.wetBulb(
      .dryBulb(
        77,
        humidityRatio: ratio,
        totalPressure: 14.175
  ...
  func test_humidityRatio_and_wetBulb_metric() async throws {
    ...
    // Above freezing
    var humidityRatio = try await client.humidityRatio(
      .wetBulb(
        .celsius(25),
        dryBulb: .celsius(30),
    ...
    // below freezing
    humidityRatio = try await client.humidityRatio(
      .wetBulb(
        .celsius(-5),
        dryBulb: .celsius(-1),
        totalPressure: .pascals(95461),
    ...
    wetBulb = try await client.wetBulb(
      .dryBulb(
        .celsius(-1),
        humidityRatio: humidityRatio,
        totalPressure: .pascals(95461),
