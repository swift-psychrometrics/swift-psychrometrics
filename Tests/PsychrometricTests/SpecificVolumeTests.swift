...
final class SpecificVolumeTests: PsychrometricTestCase {
  ...
  func test_specificVolume() async throws {
    ...
    let volume = try await client.specificVolume.moistAir(
      .dryBulb(
        75,
    ...
    let volume2 = try await client.specificVolume.moistAir(
      .dryBulb(
        86,
        humidityRatio: 0.02,
    ...
    let ratio = try await client.humidityRatio(
      .dryBulb(
        75,
        relativeHumidity: 100%,
        altitude: .seaLevel
    ...
    let volume3 = try await client.specificVolume.moistAir(
      .dryBulb(
        75,
        humidityRatio: ratio,
  ...
  func test_dry_air_metric() async throws {
    @Dependency(\.psychrometricClient) var client
    let volume = try await client.specificVolume.dryAir(
      .dryBulb(
        .celsius(24),
